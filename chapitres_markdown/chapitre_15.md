# Chapitre 15 : Etudedecas Compterlesvoyelles
...
1void (u8*s,u32 ,u32 [6])
2 u32 [26];
3 for(u32 =0;i<26;i)letters[i] 0;
5 for(u32 =0;i<size;i)
6 ++letters[s[i]-'a'];
7 }
8 v[0]letters['a'-'a'];
9 v[1]letters['e'-'a'];
...
5 %assign i i + 8
6 %endrep
#### 15.3.1.2 C’est le même principe que précédemment mais on utilisestosq, il faut donc

fixerraxà 0, sauvegarderrditemporairement (on utilise icir8) et mettre dansrcx
ouecxle nombre de répétitions :
1 xor ,rax ; rax = 0
2 mov ,rdi ; sauvegarde rdi dans r8
3 mov ,rbx ; rdi = &letters[0]
4 mov ,13 ; faire 13 fois
5 rep ;
6 movd ,r8 ; restaure rdi depuis r8
#### 15.3.1.3 On utiliseymm0qui permet de stocker 8 entiers 32 bits. On doit donc stocker
...
### 15.3.2 On commence par vérifier sisizeest égale à 0, sinon on exécute la boucle. On
...
4 xor ,rcx ; i = 0
5.while:
6 movzx ,byte[rdi+rcx]
7 inc ; ++i
8 sub ,'a' ; eax = s[i] - 'a'
9 cdqe ; convertir en 64 bits
10 inc [rbx+rax*4]
11 cmp ,rsi ; si i < size alors
12 jne ; aller en .while
13.end_while:
...
1 mov ,'a'-'a' ; stocke le nombre d'occurrences
2 mov ,rbx+r9*4]
3 mov [rdx],eax
5 mov ,'e'-'a' ; stocke le nombre d'occurrences
6 mov ,rbx+r9*4]
7 mov [rdx+4],eax
9 mov ,'i'-'a' ; stocke le nombre d'occurrences
10 mov ,rbx+r9*4]
11 mov [rdx+8],eax
15.3. TRADUCTION DE LA MÉTHODE DU TABLEAU EN ASSEMBLEUR 379
13 mov ,'o'-'a' ; stocke le nombre d'occurrences
14 mov ,rbx+r9*4]
15 mov [rdx+12],eax
17 mov ,'u'-'a' ; stocke le nombre d'occurrences
18 mov ,rbx+r9*4]
19 mov [rdx+16],eax
21 mov ,'y'-'a' ; stocke le nombre d'occurrences
22 mov ,rbx+r9*4]
23 mov [rdx+20],eax
25 mov ,rsp-8]
26 ret
...
7 movzx ,al ; r9 = s[i+1]
8 shr ,8
9 movzx ,al ; r10 = s[i+2]
10 shr ,8 ; eax = s[i+3]
12 sub ,'a' ; calcul des indices
13 sub ,'a'
14 sub ,'a'
15 sub ,'a'
17 inc [rbx+rax*4]
...
4 xor ,r8 ; nb_occ('a') = 0
5 xor ,r9 ; nb_occ('e') = 0
6 xor ,r10 ; nb_occ('i') = 0
7 xor ,r11 ; nb_occ('o') = 0
8 xor ,r12 ; nb_occ('u') = 0
9 xor ,r13 ; nb_occ('y') = 0
On remplit ensuite chacun des registresxmm2àxmm7avec respectivement des
’a’, des’e’, etc.
1 mov ,0x61616161 ; # ASCII(a) = 0x61
2 movd ,eax
3 pshufd ,xmm2,0
5 mov ,0x65656565 ; # ASCII(e) = 0x65
6 movd ,eax
7 pshufd ,xmm3,0
8 ...
10 mov ,0x79797979 ; # ASCII(y) = 0x79
11 movd ,eax
12 pshufd ,xmm7,0
...
3 jge
5 movdqu ,rdi+rcx]
6 movdqu ,xmm8 ; xmm9 = xmm8
7 movdqu ,xmm8 ; ...
8 movdqu ,xmm8
9 movdqu ,xmm8
10 movdqu ,xmm8 ; xmm13 = xmm8
12 ; 'a' ; trouver 'a'
13 pcmpeqb ,xmm2 ; comparer s[i:i+15] à ['a',...,'a']
14 pmovmskb ,xmm8 ; xmm8.b[i] = 0xFF si s[i] == 'a'
15 popcnt ,ebx ; compter le nombre de 'a'
16 add ,rbx ; ajouter au compteur de 'a'
18 ; 'e'
19 pcmpeqb ,xmm3
20 pmovmskb ,xmm9
21 popcnt ,ebx
22 add ,rbx
24 ...
26 add ,16 ; 16 prochains caractères
27 jmp
28.end_for:
...
2 mov ,0x61616161 ; 4 fois le code ASCII de 'a'
3 movd ,eax ; charger dans xmm2.d[0]
4 vpbroadcastd ,xmm2 ; recopier dans ymm2.d[1] à ymm2.d[7]
...
10 vmovdqa ,ymm8
12 ; 'a'
13 vpcmpeqb ,ymm2 ; comparaison
14 vpmovmskb ,ymm8 ; extraction
15 popcnt ,ebx ; compter les bits = compter les 'a'
16 add ,eax ; ajouter au compteur de 'a'
18 ...
19 add ,32 ; i += 32
20 jmp
21.end_for:
...
12 jz
14.for_u64:
15 vmovdqa32 ,rdi+rcx]
16 add ,64
17 vpcmpeqb ,zmm1,zmm8
18 vpcmpeqb ,zmm2,zmm8
19 vpcmpeqb ,zmm3,zmm8
20 vpcmpeqb ,zmm4,zmm8
21 vpcmpeqb ,zmm5,zmm8
22 vpcmpeqb ,zmm6,zmm8
24 ; 'a'
25 kmovq ,k1
26 popcnt ,rbx
27 add ,ebx
