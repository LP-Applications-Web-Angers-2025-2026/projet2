# Chapitre 1 : Informatique,informaticienet assembleur

Informatique,informaticienet
assembleur
...
10 int =static_cast<int>(floor(sqrt(n)));
11 for(int =3;k<=limit;k+=2)
12 if((n%k) 0)return ;
13 }
14 return ;
15}
...
1bool (int )
2 if(n<=3)return >1;
3 if(0==n%2) 0== n%3))return ;
5 for(int =5;i*i) n;i+=6)
6 if(n%i) 0) n%i+2)) 0)
7 return ;
8 }
10 return ;
...
14while(n<=N)
15 if(tab[n])
16 for(int =2*n;j<N;j+=n)tab[j]false;
17 }
18 ++n;
19}
...
8. Fowler, Noll, Vo, voirhttp://www.isthe.com/chongo/tech/comp/fnv/index.html.
