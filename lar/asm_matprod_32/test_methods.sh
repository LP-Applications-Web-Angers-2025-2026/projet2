...
for m in $methods ; do
        ...
        for i in `seq 1 $max_stats`; do
        	/usr/bin/time -f "time=%U;%E;%S;%F;%K;%M;%O" taskset -c 0 $binary $parameters -z $iterations -m $m >output.txt 2>&1
        	...
        	others=`echo $time_info | cut -d';' -f4-7`
        	echo "$user_time $elapsed_time $system_time $others" >> $stat_file
        	/usr/bin/echo -n -e "\033[s\t $i $user_time $elapsed_time $system_time $others\033[u" 
        done
