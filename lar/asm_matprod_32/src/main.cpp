...
void validity_test() {
	cout << "test" << endl;
	f32 *D = (f32 *) _mm_malloc(msize * msize * sizeof(f32), CPU_MEMORY_ALIGNMENT);
	
	CPUTimer timer;
	timer.start();
	methods[1].method(A, B, C, msize); 
	timer.stop();
	...
	for (u32 i = 2; methods[i].method != nullptr; ++i) {
		...
		memset(D, 0, msize * msize * sizeof(f32));
		timer.start();
		methods[i].method(A, B, D, msize);
		timer.stop();
		cout << i << " " << setw(25) << methods[i].name << " ";
		cout << setw(20) << timer << " ";
		int row, col;
		if (are_equal(C,D, msize,row,col)) {
			cout << " OK";
		} else {
			cout << " !!! FAIL !!! at row=" << row << ", col=" << col << " ";
			cout << D[row*msize+col] << " != " << C[row*msize+col]; 
		}
		cout << endl;
	}
	free_resources();
...
int main(int argc, char *argv[]) {
	...
	while (1) {
		...
		switch (c) {
			...
			case 's':
				msize = static_cast<u32>(atoi(optarg));
				break;
	...
	for (u32 zillion = 1; zillion <= zillions; ++zillion) {
		methods[method_id].method(A, B, C, msize);
	}
	timer.stop();
	...
	f32 result = 0.0;
	for (u32 i = 0; i < 3; ++i) {
		for (u32 j = 0; j < msize; ++j) result += c(i,j);
	}
	cout << "result=" << result << endl;
		
	if (verbose_flag) {
		print_matrix("A", A, msize);
		print_matrix("B", B, msize);
		print_matrix("C", C, msize);
	}
