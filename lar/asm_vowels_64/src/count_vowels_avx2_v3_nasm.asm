...
; -------------------------------------------------------------------	
; void cv_avx2_v3(char *s, int size, int v[6]);
; method that uses AVX2 instructions set
; rdi = s
; rsi = size
; rdx = v
; ecx = i
; r8, r9, r10, r11, r12, r13 for a, e, i, o ,u, y
; eax, ebx used for popcnt
; ymm1 to xmm6 = s[i:i+31]
; ymm8 to xmm13 are used for a,e,i,o,u,y
;
; !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
; Note that we assume that has the processor has AVX/AVX2 instructions set
; it also have the popcnt instruction
; !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
; remove dependancies whrn filling ymm8 to ymm13
; perform comparisons alltogether
cv_avx2_v3:
	mov		[rsp - 16], rbx
	mov		[rsp - 24], r12
	mov		[rsp - 32], r13
