...
; RAX = mutations
; R10d = copy of RCX
; maximum parsimony with AVX512 implementation
maxpars_avx512:
	...
	; compute number of iterations using zmm registers
	; an AVX412 register can handle 64 bytes
	mov	r10, rcx	
	shr	ecx, 6		; ecx <- (size / 64)
	test	ecx, ecx
	jz	.last63

.for_u64:
	vmovdqu8 	zmm1, [rdi + r9]		; zmm1 <- x[i:i+63]
	vpandd	zmm3, zmm1, [rsi + r9]	; zmm3 <- zmm1 & zmm2
	vpord	zmm4, zmm1, [rsi + r9]	; zmm4 <- zmm1 | zmm2

	vpcmpeqb	k1, zmm0, zmm3 		; compare zmm3 to zmm0 = [0,...,0]
									; and sets k1 consequently

	vmovdqu8	zmm3 {k1}, zmm4		; select zmm4 using k1 mask
