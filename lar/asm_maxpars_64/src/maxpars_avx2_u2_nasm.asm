...
.for_u64:
	...
	; next 32
	vmovdqa		ymm6, [rdi + r9 + 32]	; ymm1 <- x[i:i+15]
	vmovdqa		ymm7, [rsi + r9 + 32]    ; ymm2 <- y[i:i+15]

	vpand		ymm8, ymm6, ymm7    ; ymm3 <- x & y
	vpor		ymm9, ymm6, ymm7    ; xmm4 <- x | y
	
	vpcmpeqb 	ymm10, ymm0, ymm8    ; xmm8 is the mask
									; if (x[i] & y[i] == 0) 
									; then xmm5[i] = 0xFF
									; else xmm5[i] = 0x00
	vpmovmskb 	r8d, ymm10		
