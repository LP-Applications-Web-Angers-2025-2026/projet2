# Chapitre 3 : LeFonctionnementdu

## 3.1 Ce chapitre introduit les bases de l’organisation interne d’un microprocesseur
...
Central Processing Unitoù Unité Centrale de Traitement en français, remonte à
1971avec la commercialisation de l’Intel 4004en novembre de la même année.
...
d’idées entre les américainsJohn Von Neumann,John Eckert,John William
Mauchly
et le britanniqueAlan Turingdans les années 40 et 50 qui permirent
...
## 3.2 Avant de parler du microprocesseur, il est nécessaire d’évoquer le sous-système
...
dans la gestion des flux d’instructions et de données : celle deVon Neumannque
nous utilisons ici, pour laquelle données et instructions sont contenues dans une
seule mémoire, et celle dite deHarvardoù données et instructions sont stockées
...
La mémoire centrale est de typeDDRx-SDRAMoùxpeut prendre des valeurs
...
appelle cette étape unrafraîchissement mémoire. Durant cette période de
...
•
Enfin le nombrexsitué après DDR est un facteur qui définit le nombre d’octets
que l’on peut lire ou écrire lors du transfert des données.
Débit mémoire
Le débit où bande passante (bandwidthen anglais) de la mémoire est donné
par la formule suivante en Mo/s :
bandwidth= [frequency×2×2
(x−1)
]×8 (3.1)
•frequency
est la fréquence de fonctionnement de la mémoire exprimée
en MHz
•le facteur 2 correspond à la DDR qui double le taux de transfert
•xcorrespond au type de DDRx
...
théorique de100×2×2
(4−1)
×8 = 12800 Mo/s. On désigne également ce type de
...
## 3.1 Les dernières mémoires qui sont les plus utilisées en début d’année 2025 sont
...
léstimingsassociés à la fréquence mémoire et liés intrinséquement à la structure
...
barrettes mémoires appelé SPD pourSerial Presence Detect.
...
sée sous forme d’une matrice de65_536lignes et65_536colonnes car65_536 = 2
et 4 Go =2
dans un premier temps envoyer un signal RAS (Raw Access Strobe, voir Figure)
...
FIGURE3.1 – RAS, CAS, Image issue du site de Samsung
...
recommande la lecture de l’article du sitehardwaresecrets
qui traite du sujet.
Les technologies liées à la conception des mémoires centrales sont en constante
évolution (cf. Table) et sont régies par un organisme de normalisation des
semi-conducteurs appelé JEDEC (Joint Electron Device Engineering Council).
Type de mémoireAnnéeTransferts (MT/s)Débit (Go/s)
...
TABLE3.1 – Types de mémoire et caractéristiques

### 3.2.1 D’un point de vue conceptuel la mémoire SDRAM est organisée sous forme de
...
ralentissement.
Alignement mémoire
Aligner les donnéessignifie les placer à une adresse multiple d’une puissance
...
Chapitre).
Prenons l’exemple de la Figure
et où l’on a fait figurer les adresses. Lorsque l’on requête un entier de typeintqui
occupe 4 octets à l’adresse04la lecture des données peut se faire en une seule fois
car les données sont sur la même ligne. Par contre si les données sont situées sur
des lignes différentes (adresses10et19) cela implique l’envoi de deux requêtes au
contrôleur mémoire, une pour chaque ligne.
FIGURE3.2 – Bancs mémoire
...
multiple de 4, 8 ou 16. Par exemple, le compilateurgcccomporte des options en
ligne de commande comme :
•-falign-functions: aligne le début d’une fonction
•-falign-jumpset-falign-labels: aligne le code des branches du code
•-falign-loops: aligne le début des boucles

### 3.2.2 La technologie dedouble canal(cf. Figure) ou dual channel
...
FIGURE3.3 – Double ou simple canal
...
ont une longueur de plus de32_768octets.
Sur les processeurs haut de gamme on utilise aujourd’hui duquad channel, c’est
à dire quatre canaux d’accès à la mémoire voire duhexa channelsur les derniers
Intel Xeon W-3275M.

#### 3.2.2.1 On considère une mémoire de 16 Go soit2

×2
= 2
34 octets. Une barrette
de mémoire est composée de 8 chips répartis en2×4chips qui délivrent 32 bits
(4×8). En effet un chip délivre 8 bits. Au final, une barrette mémoire délivre64
bits soit2×32bits suivant deux canaux (channels) A et B (voir la Figure) .
Chaque chip stocke donc16/8 = 2Go ou autrement dit,2×8Gbits= 2
34bits.
Les 2 Go sont répartis en32bancs mémoire sous la forme de8groupes de4bancs.
Un banc stocke donc2
/32 = 2
octets, soit2
bits.
Un banc mémoire est organisé sous la forme d’une matrice de65536lignes par
8192colonnes soit2
×2
13 bits. On peut voir cette organisation sous la forme de
65536lignes de1024octets soit64Mo (voir la Figure) .
Pour accéder à un octet il faut donc au moins2
bits :
•3 bits pour le chip
•3 bits pour le groupe de bancs
•2 bits pour le banc à l’intérieur du groupe
•16 bits pour la ligne à l’intérieur du banc
FIGURE3.4 – Barrette mémoireFIGURE3.5 – Banc mémoire
...
### 3.2.3 La mémoire centrale est relativement lente par rapport au microprocesseur. Il
...
d’accès qui contient les données les plus souvent ou les plus récemment utilisées
.
On dit généralement que la mémoire cache repose sur les principes delocalité
et detemporalité. Si on accède une donnée à l’instanttà l’adressea, il y a une
forte probabilité d’accéder à l’instantt+ 1, une donnée à l’adressea+ϵ. C’est le
cas lorsque l’on écritx = x + y;ou que l’on parcourt les éléments d’un tableau.
...
(latchen anglais) composée de plusieurs transistors et non pas d’un condensateur
comme pour lesDRAM. Le rafraîchissement qui a tendance à ralentir l’accés à la
...
### 3.2.4 On distingue aujourd’hui au moins trois niveaux de cache sur les processeurs

multi-coeurs. Sur la Figure, on a fait figurer une architecture de cache pour un
...
L3 pourLevel 3. Il contient données et instructions et permet de garder la
cohérence des données entre les quatre coeurs.
. En informatique les algorithmes associés à ces techniques s’appellent LRU pourLeast Recently
Usedet LFU pourLeast Frequently Usedet sont utilisés pour remplacer les données les plus anciennes
ou les moins souvent utilisées.
FIGURE3.6 – Niveaux de mémoire cache
•
...
même cheminement dans le traitement des instructions (cf. Section)
Sur la Figure, nous avons fait le choix de montrer une organisation de
...
Section, qu’en fait il s’agit de deux fois 8 Mo.

### 3.2.5 Il existe deux organisations principales des mémoires cache lorsqu’elles doivent

coopérer. On distingue :
•
les cachesinclusifsqui sont conçus de manière à ce que toute donnée présente
Processeur / CacheL1i (ko)L1d (ko)L2 (ko)L3 (Mo)
...
TABLE3.2– Caractéristiques des caches (taille, latence, associativité) - http ://www.7-
cpu.com
...
dans L2.
•
les cachesexclusifs, pour lesquels une donnée est soit dans le cache L1, soit
...
### 3.2.6 Les mémoires caches sont organisées sous formeassociative par groupe, on

dit en anglaisn-way set associative. On peut considérer la mémoire cache comme
une table composée de blocs deklignes etmcolonnes qui stockent des données
ainsi qu’une partie de l’adresse où se trouvent les données en mémoire.
FIGURE3.7 – Cache associatif à 2 groupes
L’associativité permet d’organiser les adresses sous formes de classes d’équiva-
lence comme le montre la Figure. Imaginons que le cache contienne deux blocs
qui permettent de stocker chacun32×4octets. On dit dans ce cas que la taille de
la ligne de cache est de 4 octets et que le cache a une taille de2×32×4 = 256
octets, soit deux blocs de32lignes de4octets. On ne compte pas la partie stockant
l’adresse.

#### 3.2.6.1 Considérons pour simplifier les choses que notre microprocesseur possède un

bus d’adresses de16bits c’est à dire que les adresses ont une taille de 16 bits. Pour
stocker dans le cache l’octet situé à l’adresse48_854, on écrit l’adresse en binaire et
on la décompose ainsi :
•48_854 = 1011_1110_1101_01102
•
les deux premiers bits sont mis à 0 pour obtenir l’adresse48_852car on charge
les données par groupe de4octets, c’est à dire la taille d’une ligne de cache,
on chargera donc les octets situés aux adresses multiples de4
•
la ligne de cache où on devra placer les quatre octets est obtenue par les
5bits suivants, car il y a32lignes de cache et32 = 2
5 , soit dans notre cas
101012= 21
•
les9bits restants représentent ce que l’on appelle l’étiquette et seront stockés
dans le cache car ils permettent de reconstruire l’adresse exacte
On remarque donc pour cet exemple que toute adresse dont les bits d’indices2
à6
6ont la même valeur sera stockée dans la même ligne du cache associatif. Afin
d’améliorer l’efficacité du cache on crée doncnblocs de ce type et on essaye de
répartir au mieux les adresses entre les blocs en utilisant un algorithme de type
LRU ou LFU comme évoqué précédemment.

#### 3.2.6.2 Pour déterminer si l’octet à l’adresse41687 = 1010_0010_1101_01002 est dans

le cache, on calcule la ligne du cache où devrait se trouver l’adresse. Dans le cas
présent il s’agit de1_01012= 21 (qui correspond au bits2à6), puis on compare
en parallèle dans chaque bloc si l’étiquette1_0100_0101 est présente. Si c’est le cas,
...
cache hit. Dans le cas où elle est absente il s’agit d’uncache missoudéfaut de cache
...
## 3.3 Nous nous intéressons dans ce chapitre aux microprocesseurs de la famille x86
...
On utilise la dénomination IA-32 (pourIntel Architecture 32 bits) pour les
...
de l’électronique numérique qui décidèrent de quitter l’entrepriseFairchild Semi-
...
. On pensera également à Cyrix (1988-1997) qui a créé des clones du 80486 et du Pentium
d’Intel.
groupe d’ingénieurs et de dirigeants deFairchild Semiconductordont notamment
...
### 3.3.1 Tout comme la mémoire le microprocesseur fonctionne à une fréquence donnée
...
de coeurs
qui travaillent, comme indiqué Table.
Nombre de coeurs actifs1-23-45-1213-1617-18
Intel Core i9-7980XE 4,4 4,0 3,9 3,5 3,4
Intel Core i9-9980XE 4,5 4,2 4,1 3,9 3,8
TABLE3.3 – Modification de la fréquence en GHz en fonction du nombre de coeurs actifs
...
. Le TDP caractérise l’énergie (chaleur) dégagée par un circuit
...
refroidissement proportionné à la chaleur dégagée
.
Pour ne pas simplifier les choses, depuis l’architecture Alder Lake de 2021, Intel
définit deux valeurs de fréquence :
•le PBP pourProcessor Base Power, ce qui correspond au TDP ou au PL1
•le MTP pourMaximum Turbo Power, ce qui correspond au PL2
LeMTPreprésente la puissance ou la fréquence maximale que l’on peut atteindre
pendant une période de temps fixée, appeléetau, avant de revenir au PBP. L’exemple
donné dans l’article de Hardware Mag
auquel ce paragraphe fait référence est
celui d’un Core i5-13400 avec un PBP de 65 Watts, un MTP de 154 Watts et une
périodetaude 56 secondes.
Cependant, les microprocesseurs estampillés K comme le Core i9-13900K ont
untauinfini, et dans ce cas ont par exemple un PBP de 125 W et un MTP de 253
...
### 3.3.2 Le microprocesseur n’est en fait capable de réaliser que 3 types d’opérations :

•LOAD r,[mem]
, c’est à dire, charger dans un registrerune donnée située en
mémoire à une adresse fournie en paramètre
•STORE [mem],r
qui permet de stocker une donnée contenue dans un registre
rdans la mémoire à une adresse fournie en paramètre
•OP r3, r2, r1
oùOPest une opération arithmétique ou logique et qui signifie
mettre dans le registrer3le résultat der1 OP r2
...
dans la plupart des instructions. On note doncOP r1,r2ce qui correspond àr1 =
r1 OP r2
. Dans ce cas l’opérander1est appeléedestinationet l’opérander2est
qualifiée desource.
. On pourra consulter le sitehttps://www.anandtech.comet notamment l’articleIntel Core i7
...
CISC x86 Intel, AMD
CISC 680x0 Motorola
TABLE3.4 – Exemples d’architectures CISC er RISC
On distingue historiquement deux classes d’architectures (cf. Table) pour
les microprocesseurs :
RISC
(Reduced
Instruction Set Computer) : dans ce type d’architecture, on utilise
...
CISC
(Complex Instruction Set Computer) pour ce type d’architecture on a tendance
...
•le calcul de l’adresse mémoireA = ebx + ecx * 4 + 8
•
le chargement de la donnéeDstockée sur 4 octets à l’adresseMem[A]dans un
registre temporaireR, soitR = Mem[A]
•l’addition de la donnéeDstockée dansRavec le registreeax:R = R + eax
•l’écriture du résultat à l’adresseA,Mem[A] = R
Ce qui rend cette instructioncomplexeest le fait qu’elle combine plusieurs choses
...
. Notons que le termeReducedc’est à direréduiten français est mal choisi, on devrait plutôt
dire simplifié.
...
programmes sont constitués à 85% d’affectations, d’instructionsifet d’appels de
procédures et que 80% des affectations sont de la formevariable = valeur.
...
### 3.3.3 Si la fréquence de fonctionnement est un facteur important qui permet de
...
chipset
de la carte mère qui définit entre autres choses comment les données sont
...
Une architecture est aujourd’hui identifiée par un nom (cf. Table) qui
...
améliorer une architecture existante et le modèle a été rebaptisé PAO pourProcess
- Architecture - Optimization. Tout cela prête à confusion car il faut également prendre en compte la génération du microprocesseur (cf. Table). Il est préférable de consulter la base de données des microprocesseurs Intel afin d’obtenir des informations adéquates. . Un chipset est un ensemble de composants électroniques qui permet la communication entre microprocesseur, mémoire et périphériques. Année Etape ArchitectureGénérationFinesse 2008 Tock Nehalem 1 45 nm 2010 Tick Westmere 1 32 nm 2011 Tock Sandy Bridge 2 32 nm 2012 Tick Ivy Bridge 3 22 nm 2013 Tock Haswell 4 22 nm 2014 Optimization Haswell Refresh 4 22 nm 2014 Tick Broadwell 5 14 nm 2015 Tock Skylake 6 14 nm 2017 Optimization Kaby Lake 7 14 nm 2017 Optimization Kaby Lake Refresh 8 14 nm 2017 Optimization Coffee Lake 8,9 14 nm 2018 Optimization Whiskey Lake 8 14 nm 2019? Process Canon Lake ? 10 nm TABLE3.5 – Nom des architectures Intel en fonction des années

#### 3.3.3.1 En 1965, Gordon Moore (l’un des fondateurs d’Intel) alors ingénieur chez Fair-
...
cette seconde prédiction que l’on qualifie deloi de Mooremais qui n’est pas une loi
...
### 3.3.4 Le microprocesseur possède plusieurs variables, appelées registres, qui per-
...
à usage général identifiés par les nomseax,ebx,ecx,edxainsi que les registres
d’indexedi(Destination Index) etesi(Source Index). Il existe également des
registres qui servent à gérer la pile et à accéder aux paramètres des fonctions et
aux variables locales. Ce sontesp(Stack Pointer) etebp(Base Pointer)
.
On dénombre également plusieurs autres registres comme les registres de
segment (cs,ds,ss,es), le registreeflagsqui stocke les effets des opérations
(retenue, résultat à 0, débordement, ...) ainsi que le registreeip(Instruction
Pointer) qui stocke l’adresse mémoire de la prochaine instruction à exécuter. Pour
plus d’information sur les registres, on consultera le Chapitre.

### 3.3.5 Une adresse mémoire est définie soit par une valeur constante, soit par la valeur

d’un registre, soit par une combinaison de registres qui permet une correspondance
avec les expressions liées aux tableaux ou aux structures de données (cf. Figure).
Une adresse a donc la forme suivante :
adresse= [base+index×scale+offset]
•base
etindexsont des registres avec une restriction concernantindexqui ne
peut être le registreesp
•scale
est un facteur d’échelle et peut prendre les valeurs 1, 2, 4 ou 8 qui
...
Chacun de ces termes est optionnel. Par exemple si on désire accéder auième
élément d’un tableau d’entierston pourra choisir de stocker l’adresse detdans le
registreebx, stockeridans le registreecx, utilisereaxpour lire la valeurt[i]et
ainsi écriremov ,[ebx+ecx*4] . Le facteur d’échelle utilisé ici est 4 car un
...
gérer que 64 ko
. Chacun de ces segments est lié à la structure d’un programme :
cscorrespond au segment de code (Code Segment),dsau segment de données
(Data Segment),ssà la pile (Stack Segment) etes(Extra Segment) peut être utilisé
pour pointer sur une adresse quelconque de la mémoire.
. ESP et EBP sont généralement décrits comme faisant partie des registres d’index mais je
...
×2
= 64×1024 = 64ko.
FIGURE3.8 – Adressage mémoire
Grâce à ce mécanisme une adresse mémoire est définie par une combinaison
registre de segment et registre général ou registre d’index. Par exemple,eipest
associé àcs. Le registreediest associé àesetesiest associé àds. Les registresesp
etebpsont liés au segment de piless. Lorsque l’on accède à la prochaine instruction
au travers deip(oueipen 32 bits), on fait implicitement référence au segmentcs,
l’adresse que l’on accède est donc calculée par :
address=segment×16 +offset
= cs ×16 + ip
Le fait de multiplier par 16 le registrecsconsiste à réaliser un décalage à gauche
de 4 bits du registrecscar16 = 2
, ce qui donne une adresse sur16 + 4 = 20bits.
...
programmes et les segments seront gérés par l’assembleurnasmet le compilateur C
++.

## 3.4 Le temps d’exécution d’un programme est donné par les deux formules sui-
...
X
i=1
CPIi×Tc=N×CPIm×Tc (3.2)
La première exprime que le temps d’exécution en secondes (Te) d’un programme
deNinstructions est donné par la somme du nombre de cycles que nécessite chaque
...
moyen de cycles par instruction (CPIm) pourNinstructions :
CPIm=
N
n
X
...
On a alors au total3×2 + 1×4 + 1×10 = 20 cycles, soit un CPI moyen de20/5
...
On peut ensuite diminuer le nombre d’instructionsNou le nombre moyen de
cycles par instructions (CPIm ). Or dans ce cas, il semble que le produitN×CPIm
...
TABLE3.6 – Techniques d’amélioration des microprocesseurs
Nous allons donc passer en revue ces techniques dans la suite de ce chapitre.

## 3.5 Afin de simplifier la compréhension du traitement des instructions par le mi-

croprocesseur, on peut dire que traiter une instruction consiste à passer par cinq
étapes principales (cf. Figure) :
2.
le décodage de l’instruction afin de connaître les opérations à réaliser lors de
...
•
le tas (heapen anglais) qui représente le reste de la mémoire et c’est notam-
ment dans cette zone que l’on allouera les données grâce àmallocen C ou
newen C
++
...
FIGURE3.9 – Etapes de traitement d’une instruction
Les deux premières étapes de traitement (chargement et décodage) représentent
ce que l’on appelle lefrontal(oùfront-enden anglais), c’est à dire la partie émergée,
donc visible de l’iceberg. Les trois dernières sont qualifiées deback-enden anglais,
que l’on trouve parfois traduit pardorsal
, c’est la partie immergée et la plus
complexe.
Imaginons, de manière grossière que chacune de ces étapes prend une nano
seconde (10
−9s). Le traitement de chaque instruction demande 5 étapes d’une
nano seconde donc 5 ns. En d’autres termes, on traite une instruction toutes les 5
ns.
C’est ce que nous avons représenté sur la partie haute de la Figure. La
première instructioni1passe successivement par les 5 étapes de traitement avant
que l’instruction suivantei2puisse être traitée.

## 3.6 Afin d’améliorer la vitesse de traitement des instructions un mécanisme de
...
passe dans l’étape de chargement au tempst= 0, puis au tempst+ 1, elle passe
dans l’étape de décodage, pendant que l’instruction suivante passe dans l’étape de
FIGURE3.10 – Pipeline d’instructions
...
avec et sans pipeline pour traiterninstructions :
•
sans pipelineune instruction est exécutée toutes les 5 ns, si on aninstructions
à exécuter il faut donc5×nns.
•
avec pipeline, il faut 5 ns pour que la première instruction soit exécutée, puis
n−1ns pour exécuter lesn−1instructions restantes
...
Micro architecturePipeline Micro architecturePipeline
...
TABLE3.7 – Nombre d’étages de pipeline pour différentes architectures Intel
Un pipeline dekétapes (on parle également d’étages oùstagesen anglais),
permet théoriquement de diviser le temps de traitement park. Cependant, le
...
d’étages sur la plupart des microprocesseurs actuels (cf. Table).
Voyons à présent comment les différentes étapes de traitement des instructions
s’enchaînent.

## 3.7 Sur le schéma de la Figure

au frontal.
A partir decs:eipon obtient l’adresse de la prochaine instruction à exécuter.
Cependant comme nous allons le voir et comme cela a déjà été évoqué, certaines
instructions assembleur modifienteipet il est nécessaire d’utiliser un mécanisme de
prédiction de branchement, représenté sur la figure par BPU pourBranch Prediction
...
FIGURE3.11 – Chargement et décodage

### 3.7.1 Comme nous venons de le dire, le chargement d’une instruction fait appel à

plusieurs mécanismes dits deprédiction de branchementqui permettent de prédire
à quelle adresse le pointeur d’instruction (eip) doit se placer. Généralement il s’agit
de l’instruction suivante. Mais dans le cas de branchements, d’une boucleforpar
...
Considérons le code C de la Figure
entiers de 1 à 10. On voit sur l’organigramme de gauche qu’il exite deux chemins :
le premier est pris lorsquei≤10et le second lorsquei >10. En prévision de ce
que nous verrons dans le Chapitre, nous avons fait figurer le code assembleur
sur la même figure. Le registreeaxcontient la somme des valeurs et le registreecx
représente la variable de boucle (i).
Après l’utilisation de l’instructioncmp ecx, 10qui compare le registreecxà la
constante 10, on place une instruction de branchement conditionneljg .end_for,
qui signifiejump on greater.
...
suivi n’est pas le bon. Siecxest supérieur à 10 il faut sortir de la boucle et
modifiereippour qu’il pointe sur l’instruction après le label.end_for, c’est à
dire l’instructioni8. Cependant les instructions suivant la comparaison (i5, i6, i7)
3.7. FRONTAL : CHARGEMENT ET DÉCODAGE 117
FIGURE3.12 – Exemple de boucle for
ont déjà été chargées dans le pipeline pendant le traitement dei3eti4. On doit
...
Notons également que les conditionnelles de typeif thenouif then elseà
l’intérieur d’une boucle (forouwhile) sont les plus pénalisantes et le sont d’autant
plus qu’on ne peut prédire la condition duif(cf. Section).

### 3.7.2 Les instructions assembleur peuvent être qualifiées de macro-instructions car
...
appelées micro-opérations et notéesµ-ops.
Nous avons vu Section, l’instruction add[ebx+ecx*4+8],eax . Cette
instruction sera décomposée en plusieurs micro-opérations beaucoup plus simples
afin d’être exécutée :
1.µ-op1 : calul de l’adresseA = ebx + ecx * 4 + 8
2.µ-op2 : chargement de la donnée à l’adresse mémoireAdans le registreR
3.µ-op3 : exécution de l’additionR+eaxet stockage dansR
4.µ-op4 : stockage deRà l’adresse mémoireA
De retour à la Figure, nous voyons qu’une fois chargée dans une file
...
senté sur la figure par leµ-Ops Cache. Ce cache a pour objectif de stocker la série de
...
deµ-ops avec les données du cache, sinon on utilisera le décodeur qui est le circuit
dédié à la traduction d’une instruction x86 enµ-ops.
...
## 3.8 FIGURE3.13 – Traitement instruction

### 3.8.1 Au niveau du dorsal (voir Figure), c’est un ensemble de µ-ops associées à
...
désordre (Out Of Order) qui consiste à traiter lesµ-ops dès lors qu’elles disposent
...
Pour ce faire, on utilise deux tampons (buffers) appelésReservation Stationet
ReOrder Buffernotés respectivement RS et ROB. Nous ne détaillerons pas leur
...
soient exécutées
L’allocation avec renommage de registresest une technique essentielle pour traiter
...
### 3.8.2 Dès qu’une instruction est prête à être traitée au niveau de la RS elle est envoyée
...
la charge de travail. On qualifie cette technique d’amélioration en largeurdu
...
### 3.8.3 Après son exécution une instruction est finalement traitées par le ROB de ma-
...
### 3.8.4 Nous avons vu que l’utilisation d’un pipeline était qualifiée d’amélioration en
...
comprendre l’interaction de ces deux techniques est celle dufast foodqui est un
...
L’autre alternative qui s’offre à vous et d’aller audriveoù se trouve une longue
...
### 3.8.5 LeSimultaneous MultiThreading(SMT) est une technique qui permet le partage
...
Cette technologie qui date des années 60 a été réintroduite sur le Pentium 4
d’Intel en 2003 et a été qualifiée de technologieHyper-Threading(HT). A cette
...
### 3.9.1 Si vous utilisez Windows comme système d’exploitation vous trouverez de

nombreux outils professionnels pour obtenir des informations sur votre machine.
L’un des plus connus et les plus intéressants est CPU-Z
, il est doté d’une interface
graphique et donne de nombreux détails sur le matériel. Il existe également GPU-
Z
pour les cartes graphiques.
Sous Linux l’offre est plus restreinte et les outils commehardinfoetsysinfo
...
autre outil plus récent, appeléCPU-X, est l’équivalent de CPU-Z.
...
•lshw(list hardware)
•dmidecodedécode une table DMI (Desktop Management Interface)
•lscpu(list cpu)
•cat /proc/cpuinfo(informations sur le microprocesseur)
•lstopodu package hwloc
Par exemple pour obtenir des informations sur le microprocesseur, on peut
. Il faut noter que le Pentium 4 d’architecture Willamette date de Novembre 2000, c’est
...
utiliser la commandelscpuou, de manière équivalente, la commandelshwavec
...
(fonctionnalités) commeavx2,bmi2etpopcntpour celles qui nous intéresseront
par la suite. On trouve également le nombre de coeurs (cores=6) et le nombre de
threads (threads=12).
La lignetaillene correspond en fait à une fréquence et varie si on relance la
commande plusieurs fois. Il s’agit probablement de la fréquence de l’un des coeurs.

#### 3.9.1.1 Un petit utilitaire intéressant sous Linux estinxi. Il permet d’afficher dans
...
inxi -F -c 18
L’option-Fsignifiefullet l’option-cpermet de choisir un mode de coloration.
...
Pour utiliserlstopoil faut installer le paquethwlocsous Ubuntu :
1sudo apt install hwloc
On utilise ensuite la commandelstopooulstopo-no-graphicscomme suit :
1lstopo
2lstopo --no-io file.png
3lstopo-no-graphics -.ascii --no-io
La première commande (ligne 1) ouvre une fenêtre qui donne la topologie du
processeur (Figure) avec les informations des coeurs, l’organisation des caches
...
d’architectureSummit Ridgeest décrit comme possédant 8 coeurs, 16 threads et est
...
FIGURE3.14 – Résultat de la commande lstopo sur AMD Ryzen 7 1700X
Ce schéma indique également la numérotation des threads (P#0àP#15sur fond
gris) :
...
1ls /sys/devices/system/cpu/cpu0/cache/index*
Chaqueindexcorrespond à un cache, l’index 0 est le cache L1 de données, l’in-
...
#!/bin/sh
2input_dir="/sys/devices/system/cpu/cpu0/cache"
3levels=`ls -d ${input_dir}/index[0-9]`
4levels=`echo $levels | tr' ' '˙'| wc -l`
5level=0
6while [ $level -lt $levels ] ; do
...
14done
On peut également utiliser la commandesudo lshw -short -C memory, (voir
ci-après pour la mémoire).

### 3.9.2 Le framework logiciel DMI pourDesktop Management Interfacefournit un stan-

dard afin de gérer et de suivre les modifications de composants sur un ordinateur.
L’utilitairedmidecodepermet de lire les informations DMI de l’ordinateur et de les
...
3SMBIOS 2.8 present.
5Handle 0x0002, DMI type 2, 15 bytes
6Base Board Information
...
20Handle 0x0039, DMI type 41, 11 bytes
21Onboard Device
...
### 3.9.3 De la même manière, pour obtenir des informations concernant le sous-système

mémoire, il suffit d’utiliserdmidecode:
sudo dmidecode -t memory
2# dmidecode 3.2
3Getting SMBIOS data from sysfs.
4SMBIOS 2.8 present.
6Handle 0x000F, DMI type 16, 23 bytes
7Physical Memory Array
...
Number Of Devices: 4
15Handle 0x0017, DMI type 17, 40 bytes
16Memory Device
Array Handle: 0x000F
...
On peut également utiliser la commandelshwpour obtenir un sommaire des
différentes mémoires :
sudo lshw -short -C memory
2Chemin ... Classe Description
3=====================================================================
...
le cache L2 possède une taille de 512 ko et le cache L3 fait 32 Mo (2×16Mo).
Nous avons évoqué la présence d’informations liées au timings mémoires Sec-
tion. On peut obtenir ces informations en installanti2c-tools:
1sudo apt install i2c-tools
2sudo modprode eeprom
3decode-dimms
On obtient entre autres informations :
1EEPROM CRC of bytes 128-253 OK (0xA01C)
3---=== Memory Characteristics ===---
4Maximum module speed 2132 MHz (PC4-17000)
5Size 8192 MB
6Banks x Rows x Columns x Bits 16 x 16 x 10 x 64
7SDRAM Device Width 8 bits
8Ranks 1
9AA-RCD-RP-RAS (cycles) 15-15-15-36
10Supported CAS Latencies 16T, 15T, 14T, 13T, 12T, 11T, 10T
12---=== Timings at Standard Speeds ===---
13AA-RCD-RP-RAS (cycles) as DDR4-1866 13-13-13-31
14AA-RCD-RP-RAS (cycles) as DDR4-1600 11-11-11-27
16---=== Timing Parameters ===---
17Minimum Cycle Time (tCKmin) 0.938 ns
18Maximum Cycle Time (tCKmax) 1.600 ns
19Minimum CAS Latency Time (tAA) 13.750 ns
20Minimum RAS to CAS Delay (tRCD) 13.750 ns
21Minimum Row Precharge Delay (tRP) 13.750 ns
22Minimum Active to Precharge Delay (tRAS) 33.000 ns
23Minimum Active to Auto-Refresh Delay (tRC) 46.750 ns
24Minimum Recovery Delay (tRFC1) 350.000 ns
25Minimum Recovery Delay (tRFC2) 260.000 ns
26Minimum Recovery Delay (tRFC4) 160.000 ns
27Minimum Four Activate Window Delay (tFAW) 21.000 ns
28Minimum Row Active to Row Active Delay (tRRD_S) 3.700 ns
29Minimum Row Active to Row Active Delay (tRRD_L) 5.300 ns
30Minimum CAS to CAS Delay (tCCD_L) 5.625 ns
31Minimum Write Recovery Time (tWR) 15.000 ns
32Minimum Write to Read Time (tWTR_S) 2.500 ns
33Minimum Write to Read Time (tWTR_L) 7.500 ns
35---=== Other Information ===---
36Package Type Monolithic
37Maximum Activate Count Unlimited
38Post Package Repair One row per bank group
39Soft PPR Supported
40Module Nominal Voltage 1.2 V
41Thermal Sensor No
43---=== Physical Characteristics ===---
44Module Height 32 mm
45Module Thickness 2 mm front, 2 mm back
46Module Reference Card A revision 1
48---=== Manufacturer Data ===---
49Module Manufacturer Undefined
50Part Number Undefined
51...
...
### 3.9.4 Enfin, on dispose à présent d’un outil qui s’appelle CPU-X
...
18.04, 19.04, 19.10 et 20.04 de Ubuntu (voir Figure).
...
mkdir install
2cd install
3mv ~/Téléchargements/CPU-X_v4.0.1_Ubuntu.tar.gz .
4tar -xzf CPU-X_v4.0.1_Ubuntu.tar.gz
5cd xUbuntu_20.04
6sudo dpkg -i libcpuid15_0.5.0_amd64.deb cpuidtool_0.5.0_amd64.deb cpu-x_4.0.1_amd64.deb
7cpu-x
On peut également obtenir l’information dans le terminal en utilisant l’option
en ligne de commande–ncurses:
cpu-x --ncurses
FIGURE3.15 – Interface de CPU-X
3.10

### 3.10.1 Phoronix

est un logiciel qui permet d’installer et exécuter une série debench-
marksassez fournie.
sudo apt-get install phoronix-test-suite
2phoronix-test-suite list-available-tests
...
installer les tests qui nous intéressent comme celui desnreines :
phoronix-test-suite install n-queens
2phoronix-test-suite run n-queens
. On pourra consulterhttp://www.phoronix-test-suite.com/pour de plus amples informa-
tions.
...
2phoronix-test-suite result-file-to-csv n-queens-results
3...
4"N-Queens - Elapsed Time",16.13

### 3.10.2 1sudo apt-get install sysbench

Pour tester le CPU, on exécute le test adéquat qui calcule des décimales deπ:
1sysbench cpu --cpu-max-prime=20000 run
2sysbench 1.0.11 (using system LuaJIT 2.1.0-beta3)
4Running the test with following options:
5Number of threads: 1
6Initializing random number generator from current time
9Prime numbers limit: 20000
11Initializing worker threads...
13Threads started!
15CPU speed:
16 events per second: 182.73
18General statistics:
...
1sysbench memory --memory-block-size=256K run
2sysbench 1.0.11 (using system LuaJIT 2.1.0-beta3)
4Running the test with following options:
5Number of threads: 1
6Initializing random number generator from current time
9Running memory speed test with the following options:
block size: 256KiB
total size: 102400MiB
operation: write
scope: global
15Initializing worker threads...
17Threads started!
19Total operations: 199824 (19977.75 per second)
```

## 2149956.00 MiB transferred (4994.44 MiB/sec)

24General statistics:
...
Microprocesseur SC 32 bitsMP 32 bitsSC 64 bitsMC 64 bits
...
TABLE3.8– Résultats GeekBench 4.x.x en 32 et 64 bits SC (mono core) et MC (multi core)
Microprocesseur SC 64 bitsMC 64 bits
...
TABLE3.9 – Résultats GeekBench 5 en 64 bits SC (mono core) et MC (multi core)
...
En pratique, il faut aller sur le siteark.intel.compour obtenir les informations
de ces deux microprocesseurs. Nous avons résumé Table
comparées de ces microprocesseurs.
Processeur Core i5-7400Core i3-6100
...
TABLE3.10 – Caractéristiques des Core i5-7400 et Core i3-6100
...
moins de cache L3. Les deux architectures sont présentées Figure.
...
FIGURE3.16 – Comparaison des architectures Intel Core i3 6100 et i5 7400
...
Test NoDescription Core i5-7400Core i3-6100
...
```text
Popcnt 512333 réf. 8,05 7,54
Compte Voyelles 524288 (if) 15,50 14,47
Compte Voyelles 524288 AVX 0,85 0,79
```

TABLE3.11– Temps d’exécution en secondes de certaines études de cas pour Core i5-7400
et Core i3-6100
3.12
...
## 3.14 Exercice 18 -Quelle est la bande passante :
...
2020, le 10850K peut être trouvé au prix de 506
C, alors que le 10900K coûte
environ 600C. En général le 10850K coûte entre 50 et 150Cde moins que le
10900K.
